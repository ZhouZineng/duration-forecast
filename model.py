...
class BertTextModel_last_layer(nn.Module):
    def __init__(self):
        ...
        self.fc = nn.Sequential(nn.Linear(parsers().num_filters * len(parsers().filter_sizes), parsers().class_num*20),
                                torch.nn.ReLU(),torch.nn.Dropout(p=parsers().dropout),
                                nn.Linear(parsers().class_num*20,parsers().class_num),
                                torch.nn.ReLU(),torch.nn.Dropout(p=parsers().dropout),)
